...
class MLService:
    ...
    def predict_urgency(self, description: str) -> int:
        ...
        urgency_keywords = {
            5: ["emergencia", "grave", "crítico", "incendio", "explosión", "herida", "sangre"],
        ...
        for urgency_level in sorted(urgency_keywords.keys(), reverse=True):
            for keyword in urgency_keywords[urgency_level]:
                if keyword in description_lower:
                    logger.info(f"Detected keyword '{keyword}' -> urgency {urgency_level}")
    ...
    def validate_category(self, description: str, suggested_category: str) -> str:
        ...
        category_keywords = {
            "Movilidad": ["calle", "acera", "bache", "semáforo", "carril", "tráfico", "conducir"],
            "Limpieza": ["basura", "suciedad", "residuos", "limpio", "escombros", "polvo"],
            "Alumbrado": ["luz", "lámpara", "faro", "oscuro", "iluminación", "apagado"],
            "Seguridad": ["robo", "delito", "peligro", "seguridad", "vigilancia", "cámara"],
            "Agua": ["fugas", "agua", "inundación", "tubería", "desagüe", "alcantarilla"],
        }
